...
export default async function sell(
  ...
  // Calculate minimum salt token amount to receive (temporarily hard-coded to 90% of original value)
  const minOut = salt * 0.9;
  ...
  try {
    const { request } = await client.simulateContract({
      ...
      args: [fruit, 0], // Temporarily set to 0, should be swapped for min salt out variable
    });
    ...
    if (hash) {
      ...
      return [
        `Successfully swapped ${amount} fruit for ${valueReceived} Salt`,
