...
function httpRequest(url, opts, postBody) {
  return __async(this, null, function* () {
    yield (0, import_util.sleep)(throttleMs);
    let requestOptions = {
      method: "GET"
    };
    if (postBody != null) {
    ...
    try {
      ...
      try {
        return yield opts.onError((0, import_util.getErrorMessage)(e), void 0);
      } catch (e2) {
  ...
  const proofUrl = constructZupassPcdGetRequestUrl(
    ...
    import_semaphore_group_pcd.SemaphoreGroupPCDPackage.name,
    {
      externalNullifier: {
        ...
        userProvided: false,
        value: externalNullifier != null ? externalNullifier : (0, import_util2.generateSnarkMessageHash)(originalSiteName).toString()
      },
      group: {
        argumentType: import_pcd_types.ArgumentTypeName.Object,
        userProvided: false,
        remoteUrl: urlToSemaphoreGroup
      },
      identity: {
        argumentType: import_pcd_types.ArgumentTypeName.PCD,
        pcdType: import_semaphore_identity_pcd.SemaphoreIdentityPCDPackage.name,
        value: void 0,
        userProvided: true
      },
      signal: {
...
function useSemaphoreGroupProof(pcdStr, semaphoreGroupUrl, originalSiteName, onVerified, externalNullifier) {
  ...
  (0, import_react4.useEffect)(() => {
    if (semaphoreGroupPCD && semaphoreGroup) {
      const proofExternalNullifier = externalNullifier != null ? externalNullifier : (0, import_util2.generateSnarkMessageHash)(originalSiteName).toString();
      verifyProof(
        semaphoreGroupPCD,
        semaphoreGroup,
...
function openSemaphoreSignaturePopup(urlToZupassClient, popupUrl, messageToSign, proveOnServer) {
  const proofUrl = constructZupassPcdGetRequestUrl(
    urlToZupassClient,
    popupUrl,
    import_semaphore_signature_pcd.SemaphoreSignaturePCDPackage.name,
    {
      identity: {
        argumentType: import_pcd_types2.ArgumentTypeName.PCD,
        pcdType: import_semaphore_identity_pcd2.SemaphoreIdentityPCDPackage.name,
        value: void 0,
        userProvided: true
      },
      signedMessage: {
...
function useSemaphoreSignatureProof(pcdStr, onVerified) {
  const semaphoreSignaturePCD = useSerializedPCD(
    import_semaphore_signature_pcd.SemaphoreSignaturePCDPackage,
    pcdStr
  );
  (0, import_react5.useEffect)(() => {
    if (semaphoreSignaturePCD) {
      const { verify } = import_semaphore_signature_pcd.SemaphoreSignaturePCDPackage;
...
var FeedSubscriptionManager = class {
  ...
  pollSubscriptions() {
    return __async(this, null, function* () {
      ...
      const responses = (yield Promise.allSettled(responsePromises)).filter(import_util3.isFulfilled).flatMap((result) => result.value);
   ...
   */
  pollSingleSubscription(subscription) {
    return __async(this, null, function* () {
      const actions = yield this.fetchSingleSubscription(subscription);
      this.updatedEmitter.emit();
      return actions;
    });
  }
   ...
   */
  fetchSingleSubscription(subscription) {
    return __async(this, null, function* () {
      const responses = [];
  ...
  findSubscription(providerUrl, feed) {
    return this.activeSubscriptions.find((sub) => {
      sub.providerUrl === providerUrl && import_lodash.default.isEqual(sub.feed, feed);
    });
  }
  subscribe(providerUrl, info, credential, replace) {
    ...
    if (info.credentialType && info.credentialType !== "email-pcd" && info.credentialType !== "semaphore-signature-pcd") {
      throw new Error(
        `non-supported credential requested on ${providerUrl} feed ${info.id}`
      );
    }
    if (info.credentialType && info.credentialType !== (credential == null ? void 0 : credential.type)) {
      throw new Error(
        (credential == null ? void 0 : credential.type) ? `wrong credential type "${credential.type}" (expected "${info.credentialType}") on ${providerUrl} feed ${info.id}` : `missing credential of type "${info.credentialType} on ${providerUrl} feed ${info.id}`
      );
    }
    let sub;
    if (existingSubscription) {
  ...
  static deserialize(api, serialized) {
    ...
    return new FeedSubscriptionManager(
      api,
      (_a = parsed.providers) != null ? _a : [],
      (_b = parsed.subscribedFeeds) != null ? _b : []
    );
  }
...
function requestChangeBlobKey(zupassServerUrl, oldBlobKey, newBlobKey, uuid2, newSalt, encryptedStorage) {
  return __async(this, null, function* () {
    return httpPost(
      (0, import_url_join4.default)(zupassServerUrl, `/sync/changeBlobKey`),
      {
        onValue: () => __async(this, null, function* () {
          return { value: void 0, success: true };
        }),
        onError: (resText) => __async(this, null, function* () {
...
function requestCheckIn(zupassServerUrl, postBody) {
  return __async(this, null, function* () {
    return httpPost(
      (0, import_url_join5.default)(zupassServerUrl, "/issue/check-in"),
      {
...
function checkinTicket(zupassServerUrl, ticket, checkerIdentity) {
  return __async(this, null, function* () {
    return requestCheckIn(zupassServerUrl, {
      ticket: yield import_eddsa_ticket_pcd.EdDSATicketPCDPackage.serialize(ticket),
      checkerProof: yield import_semaphore_signature_pcd2.SemaphoreSignaturePCDPackage.serialize(
        yield import_semaphore_signature_pcd2.SemaphoreSignaturePCDPackage.prove({
          identity: {
            argumentType: import_pcd_types3.ArgumentTypeName.PCD,
            value: yield import_semaphore_identity_pcd3.SemaphoreIdentityPCDPackage.serialize(
              yield import_semaphore_identity_pcd3.SemaphoreIdentityPCDPackage.prove({
                identity: checkerIdentity
              })
            )
          },
          signedMessage: {
            argumentType: import_pcd_types3.ArgumentTypeName.String,
...
// src/api/requestCheckTicket.ts
var import_url_join6 = __toESM(require("url-join"));
function requestCheckTicket(zupassServerUrl, postBody) {
  return __async(this, null, function* () {
    return httpPost(
      (0, import_url_join6.default)(zupassServerUrl, "/issue/check-ticket"),
      {
        onValue: (resText) => __async(this, null, function* () {
          return JSON.parse(resText);
        }),
...
function requestCreateNewUser(zupassServerUrl, email, token, commitment, salt) {
  return __async(this, null, function* () {
    return httpPostSimple(
...
// src/api/requestDeviceLogin.ts
var import_url_join9 = __toESM(require("url-join"));
function requestDeviceLogin(zupassServerUrl, email, secret, commitment) {
  return __async(this, null, function* () {
    return httpPostSimple(
      (0, import_url_join9.default)(zupassServerUrl, `/account/device-login`),
      (resText) => __async(this, null, function* () {
        return {
          value: JSON.parse(resText),
          success: true
...
function requestEncryptedStorage(zupassServerUrl, blobKey) {
  return __async(this, null, function* () {
    return httpGetSimple(
      (0, import_url_join10.default)(zupassServerUrl, "/sync/load"),
      (resText) => __async(this, null, function* () {
        return {
          value: JSON.parse(resText),
          success: true
...
function requestDownloadAndDecryptStorage(zupassServerUrl, encryptionKey) {
  return __async(this, null, function* () {
    try {
      const encryptionKeyHash = yield (0, import_passport_crypto2.getHash)(encryptionKey);
      ...
      const decrypted = yield (0, import_passport_crypto2.passportDecrypt)(storageResult.value, encryptionKey);
      return {
      ...
      return { error: (0, import_util4.getErrorMessage)(e), success: false };
    }
  });
}
...
function requestIssuanceServiceEnabled(zupassServerUrl) {
  return __async(this, null, function* () {
    return httpGetSimple(
      (0, import_url_join11.default)(zupassServerUrl, "/issue/enabled"),
      (resText) => __async(this, null, function* () {
...
function requestLogToServer(zupassServerUrl, eventName, value) {
  return __async(this, null, function* () {
    try {
      yield fetch((0, import_url_join12.default)(zupassServerUrl, "client-log"), __spreadProps(__spreadValues({}, POST), {
...
function pollFeed(zupassServerUrl, identity, signedMessage, feedId) {
  return __async(this, null, function* () {
      ...
      pcd: yield import_semaphore_signature_pcd3.SemaphoreSignaturePCDPackage.serialize(
        yield import_semaphore_signature_pcd3.SemaphoreSignaturePCDPackage.prove({
          identity: {
            ...
            value: yield import_semaphore_identity_pcd4.SemaphoreIdentityPCDPackage.serialize(
              yield import_semaphore_identity_pcd4.SemaphoreIdentityPCDPackage.prove({
                identity
              })
            )
          },
...
function requestPretixSyncStatus(zupassServerUrl, isZuzalu) {
  return __async(this, null, function* () {
    return httpGetSimple(
      isZuzalu ? (0, import_url_join14.default)(zupassServerUrl, `/pretix/status`) : (0, import_url_join14.default)(zupassServerUrl, `/devconnect-pretix/status`),
      (resText) => __async(this, null, function* () {
...
function requestProveOnServer(zupassServerUrl, serverReq) {
  return __async(this, null, function* () {
    return httpPostSimple(
      (0, import_url_join15.default)(zupassServerUrl, `/pcds/prove`),
      (resText) => __async(this, null, function* () {
        return {
          value: JSON.parse(resText),
          success: true
...
function requestServerEdDSAPublicKey(zupassServerUrl) {
  return __async(this, null, function* () {
    return httpGetSimple(
      (0, import_url_join16.default)(zupassServerUrl, `/issue/eddsa-public-key`),
      (resText) => __async(this, null, function* () {
        return {
          value: JSON.parse(resText),
          success: true
...
function requestServerRSAPublicKey(zupassServerUrl) {
  return __async(this, null, function* () {
    return httpGetSimple(
      (0, import_url_join17.default)(zupassServerUrl, `/issue/rsa-public-key`),
      (resText) => __async(this, null, function* () {
        return { value: resText, success: true };
      })
    );
...
function requestUploadEncryptedStorage(zupassServerUrl, blobKey, encryptedStorage) {
  return __async(this, null, function* () {
    return httpPostSimple(
      (0, import_url_join18.default)(zupassServerUrl, `/sync/save`),
...
function requestVerifyToken(zupassServerUrl, email, token) {
  return __async(this, null, function* () {
    return httpPostSimple(
      (0, import_url_join19.default)(zupassServerUrl, `/account/verify-token`),
      () => __async(this, null, function* () {
        return { value: void 0, success: true };
      }),
      { email, token }
